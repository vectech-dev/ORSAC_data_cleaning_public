...
def sub_split_beta(ids, ratio, iter_=None):
    test_size = int(len(ids) * ratio[1])
    if iter_ != None:
        multi = iter_ * test_size
        ids = np.roll(ids, multi)
    trainlist, vallist = np.split(ids, [int(ratio[0] * len(ids))])
    return trainlist, vallist


def split_generator_beta(
    df,
    splitratio=[
        0.7,
        0.3,
    ...
    for y in ys:
        ...
        if constraint != "":
            for extra in extras:
                subsub_df = sub_df[sub_df[constraint] == extra]
                units = subsub_df[unit_col].unique()
                trainlist, vallist = sub_split_beta(units, splitratio, iter_n)
                train.extend(trainlist)
                val.extend(vallist)
                # test.extend(testlist)
        else:
            units = sub_df[unit_col].unique()
            trainlist, vallist = sub_split(units, splitratio, iter_n)
            train.extend(trainlist)
            val.extend(vallist)
            # test.extend(testlist)
